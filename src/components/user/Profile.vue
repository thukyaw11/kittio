<template>
  <v-container>
    <v-card outlined>
      <v-list-item three-line>
        <v-list-item-avatar tile size="80">
          <img :src="user.profileUrl" alt v-if="!imageEditing && user.profileUrl" />
          <img
            v-if="!user.profileUrl && !imageEditing"
            class="profile_image"
            src="/data:image/png;base64,"
            alt
          />
          <div @click="onPickFile" v-if="!imageURL">
            <img
              v-if="imageEditing"
              src="https://jbjtmarketers.org/wp-content/uploads/2017/11/upload.gif"
              alt="image upload"
            />
            <input
              type="file"
              name
              accept="image"
              style="display:none"
              ref="fileInput"
              @change="onFilePicked"
            />
          </div>
          <div v-if="imageURL" @click="onPickFile" class="profile_image">
            <img :src="imageURL" alt="preview" />
            <input
              type="file"
              name
              accept="image"
              style="display:none"
              ref="fileInput"
              @change="onFilePicked"
            />
          </div>
        </v-list-item-avatar>
        <v-list-item-content>
          <v-list-item-title class="headline mb-1" v-if="!isEditing">{{userName}}</v-list-item-title>
          <v-text-field
            v-model="newUserName"
            counter
            maxlength="20"
            required
            class="headline mb-1"
            :value="userName"
            v-if="isEditing"
          ></v-text-field>
          <v-list-item-subtitle>Myanmar</v-list-item-subtitle>
        </v-list-item-content>
      </v-list-item>

      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn text @click="editProfile" v-show="!isEditing">
          <v-icon left>mdi-pencil</v-icon>Edit
        </v-btn>
        <v-btn text v-show="isEditing" color="success" @click="changeProfile">
          <v-icon left>mdi-memory</v-icon>Save
        </v-btn>
        <v-btn text @click="editAble" v-if="!imageEditing">Edit Picture</v-btn>
        <v-btn
          v-if="imageEditing && urlValid"
          class="ma-2"
          :loading="loading"
          :disabled="loading"
          text
          @click="loader = 'loading'"
        >Upload</v-btn>
      </v-card-actions>
    </v-card>
    <v-alert type="success" v-show="alert">Your profile is updated!</v-alert>
  </v-container>
</template>
<script>
import * as firebase from "firebase";
import store from "@/store/index";
export default {
  data() {
    return {
      newUserName: store.getters["users/user"].displayName,
      userId: store.getters["users/user"].id,
      isEditing: false,
      imageEditing: false,
      imageURL: "",
      image: null,
      alert: false,
      loader: null,
      loading: false
    };
  },
  computed: {
    urlValid() {
      return this.imageEditing && this.imageURL;
    },
    user() {
      return store.getters["users/user"];
    },
    userName() {
      if (this.newUserName) {
        return this.newUserName;
      } else {
        return store.getters["users/user"].displayName;
      }
    },
    profilePost() {
      return store.getters["users/loadProfilePost"];
    }
  },
  methods: {
    onPickFile() {
      this.$refs.fileInput.click();
    },
    onFilePicked(event) {
      const files = event.target.files;

      let filename = files[0].name;
      if (filename.lastIndexOf(".") <= 0) {
        return alert("please add a valid file");
      }
      const fileReader = new FileReader();

      fileReader.addEventListener("load", () => {
        this.imageURL = fileReader.result;
      });
      fileReader.readAsDataURL(files[0]);
      this.image = files[0];
    },
    editAble() {
      this.imageEditing = !this.imageEditing;
    },
    editProfile() {
      this.isEditing = !this.isEditing;
    },

    changeProfile() {
      let currentUser = firebase.auth().currentUser;
      currentUser.updateProfile({
        displayName: this.newUserName
      });
      const newUserOptions = {
        userName: this.newUserName,
        userId: this.userId
      };
      store.dispatch("users/changeuserprofile", newUserOptions).then(() => {
        this.alert = true;
        setTimeout(() => {
          this.alert = false;
        }, 2000);
      });
      this.isEditing = !this.isEditing;
      // this.$store.dispatch("loadUser")
    },
    changeProfilePicture() {
      this.isEditing = false;
      const profileOptions = {
        userId: this.userId,
        image: this.image
      };
      store.dispatch("users/changeProfilePicture", profileOptions);
    }
  },
  watch: {
    loader() {
      this.changeProfilePicture();
      const l = this.loader;
      this[l] = !this[l];

      setTimeout(() => (this[l] = false), 3000);
      this.alert = true;
      setTimeout(() => {
        this.alert = false;
      }, 4000);

      this.loader = null;
    }
  }
};
</script>
<style scoped>
.editNameBox {
  border: none;
  border: solid 1px #ccc;
  border-radius: 5px;
  text-align: center;
  margin: 0 auto;
}
.cover-image {
  height: 200px;
  width: 100%;
  background: rgb(34, 34, 70);
}
.edit_button_cover {
  float: right;
  background: rgb(240, 238, 171, 0.2);
  border-radius: 4px;
  margin: 10px;
  margin-top: -40px;
  padding: 20px;
  cursor: pointer;
  transition: background-color 0.35s ease-in-out;
}
.edit_button_cover:hover {
  background-color: rgb(240, 238, 171, 0.5);
}
.save_button_cover {
  float: right;
  background: rgba(98, 153, 105, 0.2);
  border-radius: 4px;
  margin: 10px;
  margin-top: -40px;
  padding: 20px;
  cursor: pointer;
  transition: background-color 0.35s ease-in-out;
}
.save_button_cover:hover {
  background-color: rgba(98, 153, 105, 0.5);
}
.profile_image_container {
  width: 90px;
  height: 90px;
  margin: 0 auto;
  border: 5px solid white;
  border-radius: 50%;
  margin-top: -49px;
  background: white;
}
.profile_post_container {
  padding: 10px;
}
.profile_image {
  width: 100%;
  height: 100%;
  border-radius: 50%;
}
.profile_name {
  margin-top: 10px;
  text-align: center;
  font-size: 20px;
}
.profile_bio {
  text-align: center;
  color: grey;
}
.edit_button,
.save_button {
  width: 150px;
  border-radius: 5px;
  height: 35px;
  margin: 15px auto;
  color: white;
}
.edit_button {
  background: #4688fb;
  padding: 6px 40px;
}
.save_button {
  background: green;
  padding: 6px 60px;
}
.image_container {
  width: 99%;
  height: auto;
  margin: 0 auto;
}
.profile_gallery {
  width: 33.3%;
  padding: 3px;
  height: 120px;
}
@media screen and (min-width: 720px) {
  .profile_gallery {
    width: 33.3%;
    padding: 3px;
    height: 250px;
  }
}
</style>
