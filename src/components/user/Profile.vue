<template>
  <div>
    <a-row>
      <a-col :xs="0" :sm="6" :md="6" :lg="6" :xl="8"></a-col>

      <a-col :xs="24" :sm="12" :md="12" :lg="12" :xl="8" style="font-family: 'Roboto', sans-serif;">
        <div class="cover-image">
          <router-link to="/meetups">
            <md-icon style="padding: 25px;color: white;">arrow_back</md-icon>
          </router-link>
          <!-- edit button -->
          <div @click="editAble" v-if="!imageEditing">
            <md-icon class="edit_button_cover" style="color:yellow">edit</md-icon>
          </div>
          <!-- profile save -->
          <div @click="changeProfilePicture" v-if="urlValid">
            <md-icon class="save_button_cover" style="color:green">save</md-icon>
          </div>

        </div>
        <div class="profile_image_container" >
          <img v-if="!imageEditing && user.profileUrl"
            :src="user.profileUrl"
            alt="Avatar"
            class="profile_image"
          />
          <img v-if="!user.profileUrl && !imageEditing" class="profile_image" src="data:image/png;base64," alt="">
          <div @click="onPickFile" v-if="!imageURL">
          <img v-if="imageEditing" src="https://image.freepik.com/free-icon/upload-document_318-8461.jpg" alt="image upload" class="profile_image">
              <input
                type="file"
                name
                accept="image"
                style="display:none"
                ref="fileInput"
                @change="onFilePicked"
              />
          </div>
          <div
            v-if="imageURL"
            @click="onPickFile"
            class="profile_image"
          >
            <img :src="imageURL" alt="preview" class="profile_image" />
            <input
              type="file"
              name
              accept="image"
              style="display:none"
              ref="fileInput"
              @change="onFilePicked"
            />
          </div>
        </div>
        <div class="profile_name"></div>
        <div class="profile_name" v-show="!isEditing">{{userName}}</div>
        <p style="text-align:center">
          <input type="text" v-model="newUserName" v-show="isEditing" class="editNameBox" />
        </p>
        <div class="profile_bio">{{user.position}}</div>
        <div class="button_container" @click="editProfile">
          <div class="edit_button" v-show="!isEditing">edit profile</div>
          <div class="save_button" v-show="isEditing" @click="changeProfile">save</div>
        </div>

        <div class="profile_post_container" v-for="(profile,index) in profilePost" :key="index">
            <md-card class="md-elevation-4">
              <md-card-media>
                <img :src="profile.imageUrl" alt="People" v-if="profile.imageUrl"/>
                <img v-else src="data:image/png;base64," alt="">
              </md-card-media>

              <md-card-header>
                <div class="md-title">{{profile.petName}} ({{profile.gender}})</div>
                <div class="md-subhead">
                  <timeago :datetime="profile.createdTime"></timeago>
                </div>
              </md-card-header>

              <md-card-expand>
                <md-card-actions md-alignment="space-between">
                  <div>
                    <md-button>delete</md-button>
                    <md-button>edit</md-button>
                  </div>

                  <md-card-expand-trigger>
                    <md-button class="md-icon-button">
                      <md-icon>keyboard_arrow_down</md-icon>
                    </md-button>
                  </md-card-expand-trigger>
                </md-card-actions>

                <md-card-expand-content>
                  <md-card-content>
                    <md-list>
                      <md-list-item>
                        မျိုးရိုး
                        <span>{{profile.genetic}}</span>
                      </md-list-item>

                      <md-list-item>
                        အသက်
                        <span>{{profile.petAge}} နှစ်</span>
                      </md-list-item>

                      <md-list-item>
                        အီးမေးလ်
                        <span>{{profile.email}}</span>
                      </md-list-item>

                      <md-list-item>
                        ဖုန်း
                        <span>{{profile.phNum}}</span>
                      </md-list-item>
                    </md-list>
                    {{profile.description}}
                  </md-card-content>
                </md-card-expand-content>
              </md-card-expand>
            </md-card>

          <!-- <md-divider></md-divider> -->
        </div>
      <a-alert
      message="Your profile is updated!"
      type="success"
      show-icon
      v-show="alert"
      style="position:fixed; bottom: 20px; left:20%; z-index: 1"
    />
      </a-col>
      <a-col :xs="0" :sm="6" :md="6" :lg="6" :xl="8"></a-col>
    </a-row>
  </div>
</template>
<script>
import * as firebase from "firebase";
import store from '@/store/index'
export default {
  data() {
    return {
      newUserName: store.getters['users/user'].displayName,
      userId: store.getters['users/user'].id,
      isEditing: false,
      imageEditing: false,
      imageURL: "",
      image: null,
      alert: false
    };
  },
  computed: {
    urlValid() {
      return this.imageEditing && this.imageURL;
    },
    user() {
      return store.getters['users/user'];
    },
    userName() {
      if (this.newUserName) {
        return this.newUserName;
      } else {
        return store.getters['users/user'].displayName;
      }
    },
    profilePost() {
      return store.getters['users/loadProfilePost'];
    }
  },
  methods: {
    onPickFile() {
      this.$refs.fileInput.click();
    },
    onFilePicked(event) {
      const files = event.target.files;

      let filename = files[0].name;
      if (filename.lastIndexOf(".") <= 0) {
        return alert("please add a valid file");
      }
      const fileReader = new FileReader();

      fileReader.addEventListener("load", () => {
        this.imageURL = fileReader.result;
      });
      fileReader.readAsDataURL(files[0]);
      this.image = files[0];
    },
    editAble() {
      this.imageEditing = !this.imageEditing;
    },
    editProfile() {
      this.isEditing = !this.isEditing;
    },

    changeProfile() {
      let currentUser = firebase.auth().currentUser;
      currentUser.updateProfile({
        displayName: this.newUserName
      });
      const newUserOptions = {
        userName: this.newUserName,
        userId: this.userId
      };
      store.dispatch("users/changeuserprofile", newUserOptions).then(() => {
        this.alert = true;
        setTimeout(() => {
            this.alert = false;
        }, 2000)



      });
      // this.$store.dispatch("loadUser")
    },
    changeProfilePicture() {
      this.isEditing = false;
      const profileOptions = {
        userId: this.userId,
        image: this.image
      };
      store.dispatch("changeProfilePicture", profileOptions);
    }
  }
};
</script>
<style scoped>
.editNameBox {
  border: none;
  border: solid 1px #ccc;
  border-radius: 5px;
  text-align: center;
  margin: 0 auto;
}
.cover-image {
  height: 200px;
  width: 100%;
  background: rgb(34, 34, 70);
}
.edit_button_cover {
  float: right;
  background: rgb(240, 238, 171, 0.2);
  border-radius: 4px;
  margin: 10px;
  margin-top: -40px;
  padding: 20px;
  cursor: pointer;
  transition: background-color 0.35s ease-in-out;
}
.edit_button_cover:hover {
  background-color: rgb(240, 238, 171, 0.5);
}
.save_button_cover {
  float: right;
  background: rgba(98, 153, 105, 0.2);
  border-radius: 4px;
  margin: 10px;
  margin-top: -40px;
  padding: 20px;
  cursor: pointer;
  transition: background-color 0.35s ease-in-out;
}
.save_button_cover:hover {
  background-color: rgba(98, 153, 105, 0.5);
}
.profile_image_container {
  width: 90px;
  height: 90px;
  margin: 0 auto;
  border: 5px solid white;
  border-radius: 50%;
  margin-top: -49px;
  background: white;
}
.profile_post_container {
  padding: 10px;
}
.profile_image {
  width: 100%;
  height: 100%;
  border-radius: 50%;
}
.profile_name {
  margin-top: 10px;
  text-align: center;
  font-size: 20px;
}
.profile_bio {
  text-align: center;
  color: grey;
}
.edit_button,
.save_button {
  width: 150px;
  border-radius: 5px;
  height: 35px;
  margin: 15px auto;
  color: white;
}
.edit_button {
  background: #4688fb;
  padding: 6px 40px;
}
.save_button {
  background: green;
  padding: 6px 60px;
}
.image_container {
  width: 99%;
  height: auto;
  margin: 0 auto;
}
.profile_gallery {
  width: 33.3%;
  padding: 3px;
  height: 120px;
}
@media screen and (min-width: 720px) {
  .profile_gallery {
    width: 33.3%;
    padding: 3px;
    height: 250px;
  }
}
</style>
